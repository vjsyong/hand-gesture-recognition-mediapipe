...
def get_args():
    ...
    parser.add_argument("--min_tracking_confidence",
                        ...
                        type=float,
                        default=0.5),
    parser.add_argument("--min_classification_confidence",
                        help='min_classification_confidence',
                        type=float,
                        default=0.6)
    parser.add_argument("--webserver_address",
                        help='webserver_address',
                        type=str,
                        default="ws://4.3.2.1/ws")
...
def logging_csv(number, mode, landmark_list, point_history_list=None):
    if mode == 0:
        pass
...
def draw_info_text(image, brect, handedness, hand_sign_text, confidence, min_confidence,
                   finger_gesture_text=None):
    cv.rectangle(image, (brect[0], brect[1]), (brect[2], brect[1] - 22),
                 (0, 0, 0), -1)

    info_text = handedness.classification[0].label[0:]
    if hand_sign_text != "" and confidence > min_confidence:
        info_text = info_text + ':' + hand_sign_text + "@" + str(round(confidence, 2 - int(math.floor(math.log10(abs(confidence)))) - 1))
    else:
